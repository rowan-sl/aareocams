...
pub fn init_sender(sender: Option<ConnectionWriteHalf<Message, bincode::DefaultOptions>>) -> Subscription<ConnectionSenderEvent> {
                ...
                match r_state {
                    ...
                    ConnectionSenderState::Running(mut state) => {
                        ...
                        if let Err(e) = state.stream.send_all().await {
                            return (Some(ConnectionSenderEvent::Error(e.into())), ConnectionSenderState::Closed)
                        }
                        ...
                        (
                            None,
                            ConnectionSenderState::Running(state)
                        )
                    }
                    ConnectionSenderState::Closed => {
                        (
